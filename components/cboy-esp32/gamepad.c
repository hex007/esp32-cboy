...
{
	return (!gpio_get_level(START) << 3) |  (!gpio_get_level(SELECT) << 2) |  (!gpio_get_level(B) << 1) |  (!gpio_get_level(A) << 0);
}

uint8_t gp_get_directions()
{
	return (!gpio_get_level(DOWN) << 3) |  (!gpio_get_level(UP) << 2) |  (!gpio_get_level(LEFT) << 1) |  (!gpio_get_level(RIGHT) << 0);
}
