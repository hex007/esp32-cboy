...
static void ili_gpio_init()
{
    ...
    WRITE_PERI_REG(GPIO_ENABLE_W1TS_REG, BIT_ME(PIN_NUM_DC)|BIT_ME(PIN_NUM_RST));
}
...
void ili9341_write_frame(const uint16_t xs, const uint16_t ys, const uint16_t width, const uint16_t height, const uint16_t *data){
    int x, y;
    int i;
    uint16_t x1, y1;
    uint32_t xv, yv, dc;
